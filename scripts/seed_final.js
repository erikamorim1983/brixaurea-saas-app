...
const articles = [
    ...
    },
    {
        ...
        content_en: "<h2>The Art of Safe Leveraging</h2><p>In real estate development, profit is often not in the sale, but in the purchase and the way capital is structured.</p>",
        category: "Investment",
        read_time_minutes: 7,
        is_premium: true,
    ...
    },
    {
        ...
        title_es: "Maximizar el GDV: La Ciências del Mix de Unidades",
        ...
        summary_es: "Por qué elegir la tipología correcta es la decisión mais crítica na fase inicial de viabilidad.",
        content_pt: "<h2>O Produto Certo para o Mercado Certo</h2><p>Muitos incorporadores se apaixonam pelo projeto arquitetônico antes de validar a absorção do mercado. O segredo de um GDV (Gross Development Value) saudável está na sintonia fina entre o que o mercado local deseja e o que a legislação permite construir.</p>",
        content_en: "<h2>The Right Product for the Right Market</h2><p>Many developers fall in love with the architectural design before validating market absorption.</p>",
        category: "Market",
        read_time_minutes: 6,
        is_premium: false,
...
async function run() {
    // Delete old ones
    await fetch(`${url}/rest/v1/insights?slug=in.(ai-real-estate-2026,miami-luxury-market-trends,construction-costs-stabilization)`, {
        method: 'DELETE',
        headers: {
            'apikey': key,
    ...
    for (const article of articles) {
        // Add published_at and updated_at
        article.published_at = new Date().toISOString();
        article.updated_at = new Date().toISOString();

        const res = await fetch(`${url}/rest/v1/insights`, {
            method: 'POST',
            headers: {
                'apikey': key,
                'Authorization': `Bearer ${key}`,
